...
class _RegistrationFormState extends State<RegistrationForm> {
  ...
  Future<void> _getCurrentLocation() async {
    try {
      ...
      if (permission == LocationPermission.deniedForever) {
        _locationController.text = 'Location permissions permanently denied';
        return;
      }
      ...
      setState(() {
        _isLoading = false;
        _locationController.text = 'Error getting location';
      });
    }
  }
  ...
  Future<void> _submitForm() async {
    // First check if the form has been interacted with
    ...
    bool shouldSubmit = await showDialog(
      context: context,
      builder: (context) => AlertDialog(
  ...
  Widget _buildLocationField() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Text(
          'Click the location icon to share your current location',
          style: TextStyle(
